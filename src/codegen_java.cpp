...
std::string operand(const IrProgram& ir, const Operand& o) {
    switch (o.kind) {
        case OperandKind::None:       return "0L";
        case OperandKind::IntConst:   return std::to_string(o.intValue) + "L";
...
const char* javaBinary(Op op) {
    switch (op) {
        ...
        case Op::Sub: return " - ";
        case Op::Mul: return " * ";
        case Op::Div: return " / ";   // long / long already truncates toward zero
        case Op::Lt:  return " < ";
        case Op::Gt:  return " > ";
        case Op::Le:  return " <= ";
        case Op::Ge:  return " >= ";
...
void emitJava(const IrProgram& ir, const std::string& sourcePath,
    ...
    for (const Slot& s : ir.slots) {
        out << "        " << javaType(s.type) << " " << s.emit
            << " = " << javaZero(s.type) << ";";
        if (!s.temporary) out << "   // " << s.name << "  " << s.line << ":" << s.column;
        out << "\n";
    }
    if (!ir.slots.empty()) out << "\n";
