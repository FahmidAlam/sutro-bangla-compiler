...
bool needsDivHelper(const IrProgram& ir) {
    for (const Instr& in : ir.code) {
        if (in.op == Op::Div && in.type == Type::Int) return true;
    }
    return false;
}
...
std::string operand(const IrProgram& ir, const Operand& o) {
    switch (o.kind) {
        ...
        case OperandKind::Slot:       return ir.slots[static_cast<std::size_t>(o.slot)].emit;
    }
    return "None";
}
...
const char* pyBinary(Op op) {
    switch (op) {
        ...
        case Op::Sub: return " - ";
        case Op::Mul: return " * ";
        case Op::Div: return " / ";     // float division only; Int goes via helper
        case Op::Lt:  return " < ";
        case Op::Gt:  return " > ";
        case Op::Le:  return " <= ";
        case Op::Ge:  return " >= ";
...
void emitPython(const IrProgram& ir, const std::string& sourcePath,
    ...
    if (needsDivHelper(ir)) out << kDivHelper << "\n";
    ...
    const auto pad = [&out](int depth) {
        for (int i = 1; i < depth; ++i) out << "    ";
    };
    const auto mark = [&filled]() {
        if (!filled.empty()) filled.back() = true;
    };
    const auto closeBlock = [&](int depth) {
        ...
        if (!filled.empty()) filled.pop_back();
    };
